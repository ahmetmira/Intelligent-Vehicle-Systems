...
def detected_photo(boxes, scores, classes, detections,image):
    ...
    for (xmin, ymin, xmax, ymax), score, class_idx in zip(boxes, scores, classes):
        ...
        if score > 0:
            if class_idx in a:         # show bounding box only a classes
...
def Car_detection_video(input_video_name, output_video_name):
    ...
    success = 1
    i = 0
    while success:                                 # While there are more frames in the video
